...
class Classifier(nn.Module):
    def __init__(self, in_dim, hidden_dim, n_classes, bias=True):
        ...
        self.dec = nn.Linear(1, hidden_dim)
    ...
    def forward(self, g):
        ...
        if self.training:
            return z,F.tanh(self.classify(hg)),mu,logvar
        else:
...
for epoch in range(1000):
    ...
    for iter, (bg, label) in enumerate(data_loader):
        z,prediction,mu,logvar = model(bg)
        loss = F.cross_entropy(
            prediction, label) + F.mse_loss(z, bg.ndata['h']) + KL(mu,logvar)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
    ...
    if epoch % 10 == 0:
        with torch.no_grad():
            ...
            _,test_feat,mu,logvar = model(test_bg)
