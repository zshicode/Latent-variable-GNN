...
class Classifier(nn.Module):
    def __init__(self, in_dim, hidden_dim, n_classes, bias=True):
        ...
        self.gdata = nn.Linear(hidden_dim, 16, bias=bias)
        self.bn3 = nn.BatchNorm1d(hidden_dim)
        self.ss = Set2Set(hidden_dim,10,2)
        self.classify = nn.Linear(16, n_classes, bias=bias)
        self.hid = hidden_dim
        self.expand = False
    ...
    def forward(self, g):
        ...
        h = F.elu(self.bn1(self.conv1(g, h)))
        h = F.elu(self.bn2(self.conv2(g, h)))
...
train_bg.to(torch.device('cuda:0'))
test_X, test_Y = map(list, zip(*testset))
test_bg = dgl.batch(test_X)
test_bg.to(torch.device('cuda:0'))
test_Y = torch.tensor(test_Y).float().view(-1, 1)
train_feat,_ = model(train_bg)
test_feat,_ = model(test_bg)
train_feat = F.softmax(train_feat.cpu(),dim=1)
test_feat = F.softmax(test_feat.cpu(),dim=1)
feat = torch.cat((train_feat,test_feat),dim=0).detach().numpy()
W = buildgraph(feat.T)
# W = admm(feat.T)
f0 = np.zeros((len(trainset)+len(testset),2))
for i in range(len(trainset)):
    f0[i,trainset[i][1]] = 1
