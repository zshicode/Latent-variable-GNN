...
class Classifier(nn.Module):
    def __init__(self, in_dim, hidden_dim, n_classes, bias=True):
        ...
        # self.conv1 = SAGEConv(in_dim, hidden_dim, aggregator_type='gcn')
        ...
        # self.conv2 = SAGEConv(hidden_dim, hidden_dim, aggregator_type='gcn')
        # self.conv2 = GATConv(hidden_dim,hidden_dim,num_heads=4,feat_drop=0.3,attn_drop=0.2)
        self.bn2 = nn.BatchNorm1d(hidden_dim)
        self.mu = nn.Linear(hidden_dim, hidden_dim, bias=bias)
        self.logvar = nn.Linear(hidden_dim, hidden_dim, bias=bias)
        self.bn3 = nn.BatchNorm1d(hidden_dim)
        self.ss = Set2Set(hidden_dim,10,2)
        self.dec = GraphConv(hidden_dim, hidden_dim, bias=bias)
    ...
    def forward(self, g):
        ...
        # h = torch.eye(20)[g.in_degrees()].float().cuda() # worse
        # h = g.ndata['node_labels'].float().cuda() # worse than using degree as node feat
        # Perform graph convolution and activation function.
        h = F.elu(self.bn1(self.conv1(g, h)),alpha=1.0,inplace=True)
        g.ndata['h'] = h
        h = F.elu(self.bn2(self.conv2(g, h)),alpha=1.0,inplace=True)
        ...
        if self.expand:
            ...
            hgl = []
            for i in range(b):
                vec = torch.matmul(ubg[i].ndata['y'],ubg[i].ndata['h'])
                hgl.append(vec)
        ...
        hg = F.tanh(self.bn3(self.mu(hg)))
        if self.training:
            for i in range(b):
                ubg[i].ndata['x'] = torch.mm(ubg[i].ndata['y'].view(-1,1),hg[i].view(1,-1))
        
            g = dgl.batch(ubg)
            z = self.dec(g,g.ndata['x'])
            # return torch.sigmoid(torch.mm(z,z.t())),self.classify(hg)
            z = F.elu(self.bn4(z),alpha=1.0,inplace=True)
            return z,torch.tanh(self.classify(hg))
        else:
            return torch.tanh(self.classify(hg))
...
'''
glist, label_dict = load_graphs("ddclassification.pt")
dataset = [(glist[i],int(label_dict['glabel'][i])) for i in range(len(glist))]
...
for epoch in range(200):
    ...
    for iter, (bg, label) in enumerate(data_loader):
        adj,prediction = model(bg)
        loss = F.cross_entropy(prediction, label) + F.mse_loss(
            adj,bg.ndata['h'])
            # + F.binary_cross_entropy(adj, bg.adjacency_matrix().to_dense().cuda())
        
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        epoch_loss += loss.cpu().detach().item()
        # del bg,adj,prediction
